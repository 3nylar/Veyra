...
- Name: watch-wallet-sync.spec.ts >> watch-only wallet — sync (browser) >> Sync button re-fetches without a receiver error
- Location: tests\e2e\watch-wallet-sync.spec.ts:92:3
...
Call log:
  ...
  - waiting for locator('#connectStatus')
    14 × locator resolved to <p class="status" id="connectStatus">Base58: checksum mismatch — the string is mistype…</p>
